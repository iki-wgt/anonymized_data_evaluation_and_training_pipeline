# """
# Disclaimer:
#     Modified copy of original file from:
#     https://github.com/yhsmiley/fdet-api/blob/master/PythonAPI/pycocotools/cocoeval.py
#     All credit goes to original author(s).

#     All modifications are marked with "# modification" and a short
#     description on what is modified.

# Modification done in function summarize():
#     Added lines to output average precision of single categories
#     depending on number of categories.
#     Changed output to write category names.
# Reason of modification:
#     Wanted to support a flexible number of categories as
#     we might use only a subset of all possible classes. Also, wanted
#     to know performance on single categories
#     Subset of used classes is set before evaluation starts.
#     Changed output to write category names for easier eval.
# """
...
class COCOeval:
    ...
    def summarize(self):
        ...
        def _summarize( ap=1, iouThr=None, areaRng='all', maxDets=100 ):
            ...
            # modification: average precision depending on number of categories & print of cat name instead of cat id
            average_precision = 0
            if ap == 1:
                for i in range(0, len(p.catIds)):
                    cat_id = p.catIds[i]
                    cat_name = self.params.catIdsToCatNms[cat_id]
                    # org output: print('cat : {0} : {1}'.format(cat_name,np.mean(s[:,:,i,:])))
                    print(f"{i} {cat_name:<15} : {np.mean(s[:,:,i,:]):>0.16f}") #modification
                    average_precision += np.mean(s[:,:,i,:])
                print('mAP @ all cat : {}'.format(average_precision / len(p.catIds))) #modification
